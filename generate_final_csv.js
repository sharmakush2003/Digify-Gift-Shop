...
function parseLines(raw, baseId, dept, cat) {
  ...
  for (const line of lines) {
    ...
    let hsn = "74181010";
    let gst = 5;
    const gstMatch = text.match(/(\d{8})\s+(\d+)%/);
    if (gstMatch) {
      hsn = gstMatch[1];
    ...
    let name = text.trim();
    // remove trailing words like "Belan", "Plate" if it exists at the end
    name = name.replace(/\s+(Belan|Plate|Serving Bowl|Spice Box|Beer Mug|Copper Bottle|Glass|Napkin Stand)$/, "");
    // Replace double quotes with ' inch' to fix CSV breaking
    name = name.replace(/"/g, ' inch');
...
const allProducts = [
  ...parseLines(rawLaCoppera, 200, "Crockery & Dining", "Serveware"),
  ...parseLines(rawMashakh, 300, "Woodcraft", "Serveware")
];

// Write CSV
const header = "id,name,price,stock,department,category,subCategory,fragile,microwave,hsn,gst,description\n";
const csvRows = allProducts.map(p => {
  return `${p.id},"${p.name}",${p.price},${p.stock},"${p.department}","${p.category}","${p.subCategory}",${p.fragile},${p.microwave},"${p.hsn}",${p.gst},"${p.description}"`;
});

const csvContent = header + csvRows.join("\n");
fs.writeFileSync(path.join(__dirname, 'FINAL_PRODUCTS_IMPORT.csv'), csvContent);
console.log("Created FINAL_PRODUCTS_IMPORT.csv with " + allProducts.length + " products!");
